...
def approach_object(simulator, object_name, dynamics=False, camera_width=640, camera_height=480, center=[0. , 0. , 0.4]):
    save_path = get_save_path(simulator)
    ori_state = save_env(simulator, None)
    simulator.deactivate_suction()
    release_rgbs = []
    ...
    for t in range(release_steps):
        p.stepSimulation()
        camera_target = np.array(center)
        delta_x = 1.6 * np.cos(np.deg2rad(30)) * np.cos(np.deg2rad(180))
        delta_y = 1.6 * np.cos(np.deg2rad(30)) * np.sin(np.deg2rad(180))
        delta_z = 1.6 * np.sin(np.deg2rad(30))
        camera_position = [camera_target[0] + delta_x, camera_target[1] + delta_y, camera_target[2] + delta_z]
        simulator.setup_camera(camera_position, camera_target, camera_width=camera_width, camera_height=camera_height)
    ...
    while True:
        ...
        for normal in [random_normal, -random_normal]:
            ...
            res, path = motion_planning(simulator, mp_target_pos, target_orientation, obstacles=obstacles,
            ...
            if res:
                ...
                for idx, q in enumerate(path):
                    ...
                    delta_y = 1.6 * np.cos(np.deg2rad(30)) * np.sin(np.deg2rad(180))
                    delta_z = 1.6 * np.sin(np.deg2rad(30))
                    camera_position = [camera_target[0] + delta_x, camera_target[1] + delta_y,
                                       camera_target[2] + delta_z]
                    simulator.setup_camera(camera_position, camera_target, camera_width=camera_width,
