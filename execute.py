...
def test_env(solution_path, time_string, substeps, action_spaces, meta_info, randomize=False, obj_id=0, gui=False,
             move_robot=False, camera_height=480, camera_width=640):
    if not move_robot:
    ...
    if move_robot:
        ...
        env.primitive_save_path = save_path
        primitive_rgbs, primitive_states = approach_object(env, name, camera_width=camera_width,
                                                           camera_height=camera_height, center=center)

    rgbs, depths = take_round_images(env, camera_width=camera_width, camera_height=camera_height,
                                     center=center, distance=1.6, azimuth_interval=5)
    if move_robot:
        all_rgbs = primitive_rgbs + rgbs
    else:
        all_rgbs = rgbs
    save_numpy_as_gif(np.array(all_rgbs), "{}/{}.gif".format(save_path, "construction"), fps=10)
    run.log({"scene": wandb.Video("{}/{}.gif".format(save_path, "construction"), fps=10, format="gif")})
    # run.log({"rollout2": wandb.Video(np.array(all_rgbs), fps=10, format="gif")})
...
def execute(task_config_path, 
    ...
    if not run_training:
        test_env(solution_path, time_string, substeps, action_spaces, meta_info, randomize=randomize, obj_id=obj_id,
                 gui=gui, move_robot=move_robot, camera_height=camera_height, camera_width=camera_width)
        exit()
    ...
    run.log({"rollout": wandb.Video(np.array(all_rgbs))})
...
if __name__ == "__main__":
    ...
    parser.add_argument('--camera_height', type=int, default=480)  # whether to start from a specific state.
    parser.add_argument('--camera_width', type=int, default=640)  # whether to start from a specific state.
    args = parser.parse_args()

    path_contents = os.listdir(Path(args.task_config_path).parent)
    for item in path_contents:
        if item.startswith("task_"):
            output_name = item.split("task_")[1]
    run = wandb.init(project="Pybullet-RoboGen", name=output_name)
